...
public:
  ...
  const std::array<OwnerV4, 10000> &get_arr_own_v4() const {
    return arr_own_v4;
  }
  void set_arr_own_v4(const std::array<OwnerV4, 10000> &val) {
    arr_own_v4 = val;
  }
  ...
  const std::array<rf_owner::Owner, 10000> &get_arr_own() const {
    return arr_own;
  }
  ...
  bool load_compressed(const std::vector<uint8_t> &compressed_data) {
    ...
    std::vector<uint8_t> actual_compressed_data(
        compressed_data.begin() + offset, compressed_data.end());

    // Decompress
    std::vector<uint8_t> decompressed =
        rf_common::decompress_lz4(actual_compressed_data, original_size);
  ...
  inline size_t load_dynamic_only(const std::vector<uint8_t> &src,
                                  size_t &offset) {
    ...
    // 11. rf_owner::Owner own
    own.load_dynamic_only(src, offset);
    // 12. std::array<rf_owner::Owner, 10000> arr_own
    ...
    // 14. rf_owner::OwnerV2 own_v2
    own_v2.load_dynamic_only(src, offset);
    // 15. std::array<rf_owner::OwnerV2, 10000> arr_own_v2
    ...
    // 17. rf_owner::nested::OwnerV3 own_v3
    own_v3.load_dynamic_only(src, offset);
    // 18. std::array<rf_owner::nested::OwnerV3, 10000> arr_own_v3
    ...
    new (&vec_own_v3)
        std::vector<rf_owner::nested::OwnerV3>(std::move(vec_own_v3_tmp));
    // 20. OwnerV4 own_v4
    own_v4.load_dynamic_only(src, offset);
    // 21. std::array<OwnerV4, 10000> arr_own_v4
